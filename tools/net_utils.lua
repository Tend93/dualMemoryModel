...
--[[
take a LongTensor of size DxN with elements 1..vocab_size+1   N is batchsize D is sentences length
...
function net_utils.language_eval(predictions, id)
  ...
  utils.write_json('eval/data/val' .. id .. '.json', out_struct) -- serialize to json (ew, so gross)
  os.execute('eval/call_python_caption_eval.sh data/val' .. id .. '.json') -- i'm dying over here
  local result_struct = utils.read_json('eval/data/val' .. id .. '.json_out.json') -- god forgive me
  return result_struct
end

function net_utils.language_eval2test(predictions, id)
  -- this is gross, but we have to call coco python code.
  -- Not my favorite kind of thing, but here we go
  local out_struct = {val_predictions = predictions}
  utils.write_json('eval/test/' .. id .. '.json', out_struct) -- serialize to json (ew, so gross)
  os.execute('eval/call_python_caption_eval.sh test/' .. id .. '.json') -- i'm dying over here
  local result_struct = utils.read_json('eval/test/' .. id .. '.json_out.json') -- god forgive me
  return result_struct
end
